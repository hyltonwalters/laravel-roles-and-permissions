<x-app-layout>
    ...
    <main class="flex-1 px-8 mt-8 bg-[#222222] w-4/5">
        ...
        <h1 class="text-white text-4xl mb-10 mt-4 font-bold">{{ $user->first_name }} {{$user->last_name}}</h1>

        <form action="{{ route('users.update', $user->id) }}" method="POST" class="bg-[#303030] px-8 pt-1 pb-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4 sm:w-1/2">
                ...
                <div
                    x-data="{ open: false, selectedOptions: {{ json_encode($user->roles()->pluck('id')->toArray()) }}, roleNames: { '1': 'Admin', '2': 'Content Manager', '3': 'User' } }"
                    class="max-w-md">
            ...
            <div class="mb-4 flex flex-wrap sm:w-1/2">
                ...
                <div class="w-full md:w-1/2 md:pr-2 mb-4">
                    <x-input-label for="password" class="block text-white text-sm mb-2 font-normal">Password
                    </x-input-label>
